...
uint8_ MGPIO_u8Init(st_gpio_cfg_t* st_gpio_cfg)
{
	...
	if (st_gpio_cfg != NULL)
	{
				...
				((st_gpio_cfg->un_gpio_conf.enu_gpio_amp_mode 	< MGPIO_MAMP_INVALID) || (st_gpio_cfg->un_gpio_conf.u8_input_pull_type < MGPIO_PULL_TYPE_MAX))    )
		{
			...
			if (st_gpio_cfg->enu_gpio_mode == MGPIO_MODE_DIGITAL)
			{
							switch (st_gpio_cfg->enu_pin_dir_mode)
							{
								case MGPIO_DIR_INPUT	:	CLR_BIT(GPIODIR(loc_u8_PortNum), loc_u8_PinNum );
																				if (st_gpio_cfg->un_gpio_conf.u8_input_pull_type == MGPIO_NO_PULL)
																				{
																					CLR_BIT(GPIOPUR(loc_u8_PortNum), loc_u8_PinNum ); 
																					CLR_BIT(GPIOPDR(loc_u8_PortNum), loc_u8_PinNum );	
																				}
																				else if(st_gpio_cfg->un_gpio_conf.u8_input_pull_type == MGPIO_PULL_UP)
																				{
																					SET_BIT(GPIOPUR(loc_u8_PortNum), loc_u8_PinNum );	
																				}
																				else if(st_gpio_cfg->un_gpio_conf.u8_input_pull_type == MGPIO_PULL_DOWN)
																				{
																					SET_BIT(GPIOPDR(loc_u8_PortNum), loc_u8_PinNum );	
																				}
																				else
																				{
																					/* do nothing */
								...
								case MGPIO_DIR_OUTPUT	:	SET_BIT(GPIODIR(loc_u8_PortNum), loc_u8_PinNum );
																				...
																				if (st_gpio_cfg->un_gpio_conf.enu_gpio_amp_mode == MGPIO_OPEN_DRAIN)
																				{
																					SET_BIT(GPIOODR(loc_u8_PortNum)	, loc_u8_PinNum );
																				}
																				else if(st_gpio_cfg->un_gpio_conf.enu_gpio_amp_mode ==MGPIO_MAMP_2)
																				{
																					SET_BIT(GPIODR2R(loc_u8_PortNum), loc_u8_PinNum );
																				}
																				else if(st_gpio_cfg->un_gpio_conf.enu_gpio_amp_mode == MGPIO_MAMP_4)
																				{
																					SET_BIT(GPIODR4R(loc_u8_PortNum), loc_u8_PinNum );
																				}
																				else if(st_gpio_cfg->un_gpio_conf.enu_gpio_amp_mode == MGPIO_MAMP_8)
																				{
																					SET_BIT(GPIODR8R(loc_u8_PortNum), loc_u8_PinNum );
																				}
																				else
																				{
																					/* do nothing */
			...
			else if(st_gpio_cfg->enu_gpio_mode == MGPIO_MODE_ANALOG)
			{
				CLR_BIT(GPIODEN(loc_u8_PortNum), loc_u8_PinNum );	
			}
			else if(st_gpio_cfg->enu_gpio_mode == MGPIO_MODE_ALTF)
			{
				SET_BIT(GPIOAFSEL(loc_u8_PortNum), loc_u8_PinNum);	
			}
			else 
			{
				/* do nothing */
...
uint8_ MGPIO_u8SetPinData(enu_pin_t Copy_enPinNum, uint8_ Copy_PinValue)
{
	...
	if(Copy_enPinNum < MGPIO_PIN_INVALID && Copy_PinValue < MGPIO_PIN_MAX)
	{
		...
		if(Copy_PinValue == MGPIO_PIN_LOW)
		{
			CLR_BIT(GPIODATA(loc_u8_PortNum), loc_u8_PinNum );
		}
		else if(Copy_PinValue == MGPIO_PIN_HIGH)
		{
			SET_BIT(GPIODATA(loc_u8_PortNum), loc_u8_PinNum );	
		}
		else if(Copy_PinValue == MGPIO_PIN_TOG)
		{
		...
		Copy_enPinNum < MGPIO_PIN_INVALID)
	{
		...
		loc_u8_PinNum = Copy_enPinNum % 8;	
			...
			switch(enu_int_sens_type)
			{
				...
				case MGPIO_INT_LEVEL_SENSETIVE: SET_BIT(GPIOIS(loc_u8_PortNum), loc_u8_PinNum );
				default: ;
			}
			
			if (enu_int_sens_ctrl == MGPIO_INT_BOTH_EDGES)
			{
				SET_BIT(GPIOIBE(loc_u8_PortNum), loc_u8_PinNum );	
			}
			else if(enu_int_sens_ctrl == MGPIO_INT_FALL_E_LOW_L)
			{
				CLR_BIT(GPIOIEV(loc_u8_PortNum), loc_u8_PinNum );	
			}
			else if(enu_int_sens_ctrl == MGPIO_INT_RIS_E_HIGH_L )
			{
